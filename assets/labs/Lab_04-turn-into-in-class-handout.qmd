...
The **binomial distribution** probability and cdf in R:
...
When working with the Binomial distribution, you can use the fact that if $Y \sim binom(n,p)$ then $E(Y) = np$ and $V(Y) = np(1-p)$ We will prove this later on in the course.
