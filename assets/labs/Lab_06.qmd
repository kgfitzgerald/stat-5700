...
As you'll recall from the Binomial distribution handout, R includes many built in functions for common distributions. Commands for each follow the same basic format. The abbreviated name of the distribution is preceded by once of four letters:
...
For this lab, you will (re-)do odd problems from HW 06, but rather than solving by hand, you will use the `dgeom()`, `dnbinom()`, `dhyper()`, and `dpois()` to obtain the answer.
